...
def train(device, epoch, params, model, optimizer, Tot_epoch, loss_array, x,
          y):
    ...
    if (epoch + 1 == hyperparameters.epochs):
        ...
        for i in range(sample):
            ...
            res2[i, 4] = preds.detach().cpu().numpy()[(2 * i + 1) *
                                                      size:(2 * i + 2) * size]
        res.tofile('./data/exp_single_cond_' + str(int(x * 1000)) + '_' +
                   str(int(y * 1000)) + '_' + str(epoch + 1) + '.dat')
        res2.tofile('./data/exp_single_L11_L12_' + str(int(x * 1000)) + '_' +
                    str(int(y * 1000)) + '_' + str(epoch + 1) + '.dat')
