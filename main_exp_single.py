...
# own modules
import hyperparameters.hyperparameters_exp_single as hyperparameters
import NN.NN_mu_constant as NN
import dataloader.dataloader_exp_single as dataloader
import utils
...
def train(device, epoch, params, model, optimizer,Tot_epoch,loss_array,x,y):
    ...
    if (epoch+1==hyperparameters.epochs):
        ...
        res.tofile('./data/exp_single_cond_'+str(int(x*1000))+'_'+str(int(y*1000))+'_'+str(epoch+1)+'.dat')
        res2.tofile('./data/exp_single_L11_L12_'+str(int(x*1000))+'_'+str(int(y*1000))+'_'+str(epoch+1)+'.dat')
    ...
    for epoch in range(hyperparameters.epochs):
        ...
        if epoch%100==0:
            with utils.Timer('epoch {} timing: '.format(epoch)):
                train(device, epoch, hyperparameters.params, model, optimizer,hyperparameters.epochs,loss_array,hyperparameters.x,hyperparameters.y)
            print('--'*10)
        else:
            train(device, epoch, hyperparameters.params, model, optimizer,hyperparameters.epochs,loss_array,hyperparameters.x,hyperparameters.y)      
    loss_array.tofile('./data/exp_single_loss_'+str(int(hyperparameters.x*1000))\
                +'_'+str(int(hyperparameters.y*1000)) +'_'+str(epoch+1)+'.dat')