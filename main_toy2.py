...
# own modules
import hyperparameters.hyperparameters_toy2 as hyperparameters
import NN.NN_mu_all as NN
import dataloader.dataloader_toy2 as dataloader
import utils
...
def train(device, epoch, params, model, optimizer,Tot_epoch,loss_array):
    ...
    if (epoch+1==Tot_epoch):
        ...
        for i in range(sample):
            ...
            chem=chem_Q[i*3]+0*chem_Q[i*3+1]*res2[i,0]+chem_Q[i*3+2]*res2[i,0]**2
            res2[i,5]=np.arctan(chem)/(np.pi/2.0)*Wc
        res.tofile('./data/toy_cond_'+str(epoch+1)+'_'+str(sample)+'.dat')
        res2.tofile('./data/toy_L11_L12_'+str(epoch+1)+'_'+str(sample)+'.dat')
    ...
    for epoch in range(hyperparameters.epochs):
        ...
        if epoch%100==0:
            with utils.Timer('epoch {} timing: '.format(epoch)):
                train(device, epoch, hyperparameters.params, model, optimizer,hyperparameters.epochs,loss_array)
            print('--'*10)
        else:
            train(device, epoch, hyperparameters.params, model, optimizer,hyperparameters.epochs,loss_array)
            
            
    loss_array.tofile('./data/toy_loss_'+str(epoch+1)+'_'+str(sample)+'.dat')        
