...
class Dataset(data.Dataset):
    ...
    def __init__(self, ):
        ...
        for i in range(inputs.shape[0]):
            ...
            newEB=newE+muB[i]
            newEC=newE+muC[i]
            newYA=2.*torch.exp(-((newEA-2))**2)+torch.exp(-((newEA+2))**2)
            newYB=2.*torch.exp(-((newEB-2))**2)+torch.exp(-((newEB+2))**2)
            newYC=2.*torch.exp(-((newEC-2))**2)+torch.exp(-((newEC+2))**2)
            
            #calculate L11 and L12 for each sample
            newfA_1=newYA*torch.exp((newE)/inputs[i])/(torch.exp((newE)/inputs[i])+1)**2/inputs[i]
            newfA_2=(newE)*newYA*torch.exp((newE)/inputs[i])/(torch.exp((newE)/inputs[i])+1)**2/inputs[i]
            newfB_1=newYB*torch.exp((newE)/inputs[i])/(torch.exp((newE)/inputs[i])+1)**2/inputs[i]
            newfB_2=(newE)*newYB*torch.exp((newE)/inputs[i])/(torch.exp((newE)/inputs[i])+1)**2/inputs[i]
            newfC_1=newYC*torch.exp((newE)/inputs[i])/(torch.exp((newE)/inputs[i])+1)**2/inputs[i]
            newfC_2=(newE)*newYC*torch.exp((newE)/inputs[i])/(torch.exp((newE)/inputs[i])+1)**2/inputs[i]            
