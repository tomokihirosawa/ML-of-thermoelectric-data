...
class FCN(nn.Module):
    ...
    def __init__(self, fcs):
        ...
        n_fc = len(fcs)-1
        for i in range(n_fc):
            fc_layers.extend( [nn.Linear(fcs[i], fcs[i+1]),
                self.actfunc]
                )
        ...
        self.parameter1 = nn.Parameter(torch.randn(1,)*0.0, requires_grad=True)
    ...
    def forward(self, x,Wc,alpha,cond,Delta):
        #x is input of NN, which is $\xi_i=T_i/T_N$ for i=1,2,...,N
        #Wc is cutoff for energy integrals
        #alpha is the chemical potential obtained by NN_mu_all
        #cond is the spectral conductivity obtained by NN_mu_all
        
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        size=x.shape[0]
        energy=(torch.linspace(-2*Wc, 2*Wc, 4*size+1)).to(device)
        out=torch.zeros(2*size+1).to(device)
        
        Q=self.fc_layers(x)[:4*size+10]
        # averaging 5 consequtive elements of the output vector to compute the spectral conductivity
        Sigma=torch.zeros(4*size+1).to(device)
        for j in range(Sigma.shape[0]):
            Sigma[j]=(Q[j]+Q[j+1]+Q[j+2]+Q[j+3]+Q[j+4]+Q[j+5]+Q[j+6]+Q[j+7]+Q[j+8]+Q[j+9])/10.0
        # add the sample-dependent corrections in the spectral conductivity
        Sigma=cond+Sigma
        
        #prepare interpolation of sigma(E) for energy integrals
        gi2 = torch_interpolations.RegularGridInterpolator([energy],Sigma)
        
        ## add a small chemical potential
        chem_Q=self.fc_layers(x)[4*size+10:]
        chem=alpha+torch.atan(chem_Q)/(np.pi/2.0)*Wc*Delta
        for i in range(size):
            #interpolate sigma(E) between -Wc*x[j]+mu and Wc*x[j]+mu.             
            newE=torch.linspace((-Wc*x[i]).item(),(Wc*x[i]).item(),101).to(device)
            newE=newE+chem[i]
            newSigma=gi2([newE])
            newE=newE-chem[i]      

            #calculate L11 and L12            
            newf1=newSigma*torch.exp((newE)/x[i])/(torch.exp((newE)/x[i])+1)**2/x[i] 
            newf2=newE*newSigma*torch.exp((newE)/x[i])/(torch.exp((newE)/x[i])+1)**2/x[i] 
