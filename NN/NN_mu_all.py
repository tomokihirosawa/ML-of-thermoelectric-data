    ...
    def forward(self, x,Wc,Nsample):
        ...
        for i in range(Nsample):           
            for j in range(size):
                ...
                chem=chem_Q[i*3]+0*chem_Q[i*3+1]*x[j]+chem_Q[i*3+2]*x[j]**2     
                chem=torch.atan(chem)/(np.pi/2.0)*Wc
                
                #interpolate sigma(E) between -Wc*x[j]+mu and Wc*x[j]+mu.             
                newE=torch.linspace((-Wc*x[j]).item(),(Wc*x[j]).item(),101).to(device)
                newE=newE+chem
                newSigma=gi2([newE])
                newE=newE-chem    
                
                #calculate L11 and L12 
                newf1=newSigma*torch.exp((newE)/x[j])/(torch.exp((newE)/x[j])+1)**2/x[j] 
                newf2=newE*newSigma*torch.exp((newE)/x[j])/(torch.exp((newE)/x[j])+1)**2/x[j] 
