...
def train(device, epoch, params, model, optimizer, Tot_epoch, loss_array, x, y,
    ...
    if (epoch + 1 == hyperparameters.epochs):
        ...
        chemQ = O[size * 4 + 10:]
        chem = alpha.detach().cpu().numpy() + np.arctan(chemQ) / (
            np.pi / 2.0) * Wc * Delta
        ...
        res.tofile('./data/exp_focus_cond_' + str(int(x * 1000)) + '_' +
                   str(int(y * 1000)) + '_' + str(epoch + 1) + '_delta_' +
                   str(int(Delta * 1000)) + '.dat')
        res2.tofile('./data/exp_focus_L11_L12_' + str(int(x * 1000)) + '_' +
                    str(int(y * 1000)) + '_' + str(epoch + 1) + '_delta_' +
                    str(int(Delta * 1000)) + '.dat')
...
if __name__ == '__main__':
    ...
    epoch = 5000
    sample = 7
    data1 = np.fromfile('./data/exp_all_cond_' + str(epoch) + '_' +
                        str(sample) + '.dat',
                        dtype=np.float64,
                        count=5000)
    data2 = np.fromfile('./data/exp_all_L11_L12_' + str(epoch) + '_' +
                        str(sample) + '.dat',
                        dtype=np.float64,
                        count=7 * 6 * 1000)
    nsite = len(data1) // 2
    ...
    ind = np.where((xarray == hyperparameters.x)
                   & (yarray == hyperparameters.y))[0]
        ...
        if epoch % 100 == 0:
            with utils.Timer('epoch {} timing: '.format(epoch)):
                train(device, epoch, hyperparameters.params, model, optimizer,
                      hyperparameters.epochs, loss_array, hyperparameters.x,
                      hyperparameters.y, alpha, cond, Delta)
            print('--' * 10)
        else:
            train(device, epoch, hyperparameters.params, model, optimizer,
                  hyperparameters.epochs, loss_array, hyperparameters.x,
